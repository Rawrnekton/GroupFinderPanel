...
class ProtoClient implements Runnable {
	...
	public void run() {
		...
		/*
		 * schreibeNachricht(socket, zuSendendeNachricht); empfangeneNachricht =
		 * leseNachricht(socket);
		 */

		/*
		 * socket.close(); System.out.println("Socket closed");
		 */
	}
	...
	protected void schreibeNachricht(Socket socket, int clientID) {
		System.out.println("Writing clientID to socket.");
		try {
			outToServerStream.writeInt(clientID);
		} catch (IOException e) {
			e.printStackTrace();
			System.out.println("Socket konnte nicht mit ID beschrieben werden.");
		}
		System.out.println("Writing clientID was succesfull.");
	}
